...
\hypersetup{
            ...
            pdfauthor={가톨릭의대 약리학교실 연구강사 한성필},
            colorlinks=true,
            linkcolor=Maroon,
            citecolor=Blue,
            urlcolor=Blue,
...
(Non-compartmental analysis, NCA)입니다. \emph{식약처, FDA (Food and
Drug Administration)를 비롯한 대부분의 규제기관에서는 NCA하는
소프트웨어를 규정하고 있지 않아}, 상용 소프트웨어를 사용하지 않고
약동학적 지표를 구하는 것을 허용하고 있습니다. 따라서 무료로 누구나
사용할 수 있는 R 패키지를 사용하여 비구획분석을 통한 약동학적 주요
지표를 구할 수 있습니다.
...
\item
  NonCompart (Bae
  \protect\hyperlink{ref-R-NonCompart}{2019}\protect\hyperlink{ref-R-NonCompart}{b})
\item
  ncar (Bae
  \protect\hyperlink{ref-R-ncar}{2019}\protect\hyperlink{ref-R-ncar}{a})
\item
...
\section{R에 대하여}\label{ruxc5d0-uxb300uxd558uxc5ec}

R (R Core Team \protect\hyperlink{ref-R-base}{2019})은 통계 소프트웨어
...
\section{자료 탐색}\label{uxc790uxb8cc-uxd0d0uxc0c9}
...
\texttt{read.csv()} 함수를 사용해서 자료를 불러 옵니다. 엑셀 파일을
사용하는 경우 \texttt{readxl} 패키지를 설치한 후에
\texttt{read\_excel()} 함수를 사용해서 불러올 수 있습니다. 다만 이 경우
\texttt{tibble} 형태로 자료가 변형되므로 \texttt{as.data.frame()}을
사용해서 데이타프레임으로 변형해주어야 합니다.
...
\chapter{패키지: NonCompart}\label{uxd328uxd0a4uxc9c0-noncompart}

\section{tblNCA(): 전체 대상자 비구획
분석}\label{tblnca-uxc804uxccb4-uxb300uxc0c1uxc790-uxbe44uxad6cuxd68d-uxbd84uxc11d}
...
결과는 \texttt{data.frame} 형태인데 너무 길기 때문에 핵심적인 일부
파라메터 (C\textsubscript{max}, T\textsubscript{max},
AUC\textsubscript{last})만 표시할 수도 있습니다.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Theoph_tblNCA_selected <-}\StringTok{ }\NormalTok{Theoph_tblNCA }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{select}\NormalTok{(Subject, CMAX, TMAX, AUCLST) }\OperatorTok{%>%}\StringTok{ }
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Indometh_tblNCA_selected <-}\StringTok{ }\NormalTok{Indometh_tblNCA }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{select}\NormalTok{(Subject, CMAX, TMAX, AUCLST) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{print}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
statistics)}\label{uxae30uxc220uxd1b5uxacc4-descriptive-statistics}

R에서는 필요에 따라서 자신만의 함수를 만들 수도 있습니다. 아래를
실행하면 \texttt{desc\_tblNCA()} 함수를 사용하여 기술통계량을 쉽게 구할
수 있습니다. (Table \ref{tab:theodesc} and \ref{tab:indodesc})

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{desc_tblNCA <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(tblNCA)\{}
\NormalTok{  psych}\OperatorTok{::}\KeywordTok{describe}\NormalTok{(tblNCA) }\OperatorTok{%>%}
\StringTok{    }\KeywordTok{select}\NormalTok{(n, mean, sd, median, min, max)}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\midrule
Subject* & 12 & 6.500000 & 3.605551 & 6.50000 & 1.00000 & 12.000\\
CMAX & 12 & 8.759167 & 1.472959 & 8.46500 & 6.44000 & 11.400\\
TMAX & 12 & 1.788333 & 1.112408 & 1.13500 & 0.63000 & 3.550\\
AUCLST & 12 & 103.806775 & 23.645216 & 95.40665 & 73.77555 & 148.923\\
\bottomrule
\end{tabular}
\end{table}
...
\toprule
  & n & mean & sd & median & min & max\\
\midrule
Subject* & 6 & 3.500000 & 1.8708287 & 3.500 & 1.00000 & 6.00000\\
CMAX & 6 & 2.076667 & 0.4135537 & 2.040 & 1.50000 & 2.72000\\
TMAX & 6 & 0.250000 & 0.0000000 & 0.250 & 0.25000 & 0.25000\\
AUCLST & 6 & 2.485208 & 0.5267325 & 2.675 & 1.74125 & 2.93375\\
\bottomrule
\end{tabular}
\end{table}

\chapter{패키지: ncar}\label{uxd328uxd0a4uxc9c0-ncar}
...
\chapter{패키지: pkr}\label{uxd328uxd0a4uxc9c0-pkr}
...
\begin{figure}
\includegraphics[width=0.95\linewidth]{Output/PK_Profile_Linear_Scale_for_Theoph} \caption{평균 약동학 파라메터와 그룹 농도-시간 그림 (선형)}\label{fig:unnamed-chunk-21}
\end{figure}

\begin{figure}
\includegraphics[width=0.95\linewidth]{Output/PK_Profile_Log_10_Scale_for_Theoph} \caption{평균 약동학 파라메터와 그룹 농도-시간 그림 (로그)}\label{fig:unnamed-chunk-22}
\end{figure}

\begin{figure}
\includegraphics[width=0.95\linewidth]{Output/PK_Profile_with_CI_for_Theoph} \caption{평균 약동학 파라메터와 그룹 평균 농도-시간 그림 (로그)}\label{fig:unnamed-chunk-23}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.95\linewidth]{Output/Individual_PK_Linear_Scale_for_Theoph} \caption{약동학 파라메터와 함께 표시되는 농도-시간 그림 (선형)}\label{fig:unnamed-chunk-24}
\end{figure}

\begin{figure}
\includegraphics[width=0.95\linewidth]{Output/Individual_PK_Log_10_Scale_for_Theoph} \caption{약동학 파라메터와 함께 표시되는 농도-시간 그림 (로그)}\label{fig:unnamed-chunk-25}
\end{figure}

\chapter{기타 사항}\label{etc}

\section{shiny 앱}\label{shiny-uxc571}
...
\section{지원}\label{uxc9c0uxc6d0}

패키지와 관련한 모든 의문은
\href{mailto:shan@catholic.ac.kr}{\nolinkurl{shan@catholic.ac.kr}} /
02-2258-7882 으로 연락 주시면 빠르게 도움 드리겠습니다. 혹은
StackOverflow\footnote{\url{https://stackoverflow.com}}에 영어로 질문
올려주시고 링크를 보내주시면 더 좋습니다. 아직 미완성이지만 Gitbook
(일종의 웹북)\footnote{\url{https://asancpt.github.io/book-ncar}}을 통해
전자출판도 진행 중이므로 시간 나실때 틈틈이 확인해 주시면
감사하겠습니다. (Figure \ref{fig:gitbook}) 이 실습자료도
Gitbook\footnote{\url{https://asancpt.github.io/handout}}으로 열람
가능합니다.

가톨릭의대 약리학교실, 가톨릭계량약리학연구소 연구강사 한성필

\section{고지}\label{uxace0uxc9c0}

본 출판물은 2016-2019년도 정부(미래창조과학부)의 재원으로 한국연구재단
...
Bae, Kyun-Seop. 2019a. \emph{Ncar: Noncompartmental Analysis for
Pharmacokinetic Report}. \url{https://CRAN.R-project.org/package=ncar}.

\hypertarget{ref-R-NonCompart}{}
---------. 2019b. \emph{NonCompart: Noncompartmental Analysis for
