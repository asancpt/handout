...
```{r}
Theoph_tblNCA_selected <- Theoph_tblNCA[ , c('ID', 'CMAX', 'TMAX', 'AUCLST')]
...
```{r}
Indometh_tblNCA_selected <- Indometh_tblNCA[ , c('ID', 'CMAX', 'TMAX', 'AUCLST')]
...
```{r pdfNCA}
pdfNCA(fileName="Output-ncar/pdfNCA-Theoph.pdf", Theoph, colSubj="Subject", 
...
```{r eval = FALSE}
rtfNCA(fileName="rtfNCA-Theoph.rtf", Theoph, colSubj="Subject", 
       colTime="Time", colConc="conc", dose=320, doseUnit="mg", 
       timeUnit="h", concUnit="mg/L")
```
...
이 실습자료도 Gitbook^[https://asancpt.github.io/handout-ncar]으로 열람 가능합니다.
